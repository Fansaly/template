...
export default {
  ...
  computed: {
    ...mapState({
      hostSDKVersion: (state) => state.app.hostSDKVersion,
  ...
  methods: {
    ...
    async handleSignin() {
      ...
      if (compareVersion.gte(this.hostSDKVersion, '2.27.1')) {
      ...
      try {
        let err, res;
        [err, res] = await uni.login({ provider: 'weixin' });
        if (err) {
          throw err;
        }
        const code = res.code;

        [err, res] = await uni.getUserProfile({
          desc: 'get user info',
          lang: 'zh_CN',
        });
        if (err) {
        ...
        await this.signin({ ...res.userInfo, code });
