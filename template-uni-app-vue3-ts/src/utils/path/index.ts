export interface Route extends Page.PageInstanceBaseProps {
  [key: string]: any;
}
...
export const getCurrentPage = () => {
  const pages: Route[] = [...getCurrentPages()];
...
export const transformPath = (fullpath: string) => {
  const re = /^(.*?)(\?(.*))?$/;
  const matches = fullpath.match(re) as Array<string>;
  const to = matches[1];
...
export const resolvePath = ({ to, from }: { to: string; from?: string }) => {
  if (/^\//.test(to)) {
    return to;
  }
